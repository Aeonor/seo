...
$(function() {
  ...
  $('#cache a.admin-button').data('active', false).on('click', function() {
    if ($(this).data('active')) {
      ...
      $('body').addClass('content-editable');
    }
  ...
  // Congrès 
  if ( $('#congres').size() > 0 ) {
    var congresPast = $('.link-list:first');
    var congresFuture = $('.link-list:last');
    $('<button class="content-editable btn btn-primary">Ajouter un congrès</button>')
    .on('click', function() {
      $('<a href="#"><strong>Nom du congrès</strong><br />Description du congrès</a>').appendTo(congresPast);
    })
    .insertAfter(congresPast);
    $('<button class="content-editable btn btn-primary">Ajouter un congrès</button>')
    .on('click', function() {
